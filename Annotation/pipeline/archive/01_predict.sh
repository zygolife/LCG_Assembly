...
IFS=,
sed -n ${N}p $SAMPFILE | while read ProjID JGISample JGIProjName JGIBarcode SubPhyla Species Strain Note
do
 ...
 pushd $name.predict.$$
    funannotate predict --cpus $CPU --keep_no_stops --SeqCenter JGI --busco_db fungi_odb9 --strain "$Strain" \
      -i ../$INDIR/$name.masked.fasta --name $JGIProjName --protein_evidence ../lib/informant.aa \
      -s "$species"  -o ../$OUTDIR/$name
 rmdir $name.predict.$$
done
